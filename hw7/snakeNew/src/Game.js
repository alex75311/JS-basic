class Game {
    ...
    /**
     * Метод изменяет скорость змейки
     */
    changeSpeed() {
        this.settings.speed = this.snake.lenSnake() / 2;
        clearInterval(this.tickIdentifier);
        this.tickIdentifier = setInterval(this.doTick.bind(this), 1000 / this.settings.speed);
    }
     ...
     */
    doTick() {
        // this.setMessage(this.snake.lenSnake());
        this.setScore(`Скорость: ${this.settings.speed}
                        Счет: ${this.snake.lenSnake()}
                        Чтобы выиграть необходимо набрать: ${this.settings.winLength}`);
