class Board {
    ...
    isNextStepToWall(nextCellCoords) {
        ...
        if (nextCellCoords.x > this.settings.rowsCount) {
            this.snake.body[0].x = 1
        } else if (nextCellCoords.x < 1) {
            this.snake.body[0].x = this.settings.rowsCount
        } else if (nextCellCoords.y > this.settings.colsCount) {
            this.snake.body[0].y = 1
        } else if (nextCellCoords.y < 1) {
            this.snake.body[0].y = this.settings.colsCount
        }
    }

    /**
     * Является ли следующий шаг, шагом в тело змеи.
     * @param {Object} nextCellCoords - координаты ячейки, куда змейка собирается сделать шаг.
     * @param {number} nextCellCoords.x
     * @param {number} nextCellCoords.y
     * @returns {boolean}
    ...
    isNextStepToBody(nextCellCoords) {
        let answer = false
        this.snake.body.forEach(function (body, i) {
            if (nextCellCoords.x == body.x && nextCellCoords.y == body.y && i != 0) {
                answer = true
            }
        })
        return answer
